...
class AdminStack(cdk.Stack):
    def __init__(
        ...
        **kwargs,
        ...
        s3_bucket_origin_id = f"{SOE}-admin-S3-bucket"
        api_gateway_origin_id = f"{SOE}-server-api"

        id = f"{SOE}-admin-s3-bucket"
        admin_s3_bucket = s3.CfnBucket(
            self,
            id,
            bucket_name=f"{SOE}-admin",
        ...
        id = f"{SOE}-admin-s3-bucket-policy"
        s3.CfnBucketPolicy(
            self,
            id,
            bucket=admin_s3_bucket.ref,
            policy_document={
                "Version": "2012-10-17",
                "Statement": [
                    {
                        ...
                        "Principal": "*",
                        "Action": "s3:GetObject",
                        "Resource": f"arn:aws:s3:::{SOE}-admin/*",
                    }
                ],
            },
        )

        id = f"{SOE}-admin-cloudfront-distribution"
        cloudfront_distribution = cloudfront.CfnDistribution(
            ...
            distribution_config=cloudfront.CfnDistribution.DistributionConfigProperty(
                ...
                aliases=[f"{admin_subdomain}{domain}"] if add_aliases == "yes" else [],
                origins=[
                    ...
                    cloudfront.CfnDistribution.OriginProperty(
                        ...
                        domain_name=f"{admin_s3_bucket.bucket_name}.s3-website.{region}.amazonaws.com",
        ...
        cloudfront_distribution.add_depends_on(admin_s3_bucket)
